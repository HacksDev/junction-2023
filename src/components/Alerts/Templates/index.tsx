...
import { TemplateData, TemplateIcon, initialTemplatesData } from "../data";
...
const icons: Record<TemplateIcon, ReactNode> = {
  contacts: <ContactsIcon />,
  default: <InfoIcon />,
  portfail: <PortfailIcon />,
};
...
export default function Templates() {
  ...
  useEffect(() => {
    if (!templates || !Object.entries(templates).length) {
    ...
    <>
      {templates &&
        Object.entries(templates)?.map(([id, template]) => (
          <Template key={id} id={id} {...template} icon={icons[template.icon]} />
        ))}
    </>
  );
}
